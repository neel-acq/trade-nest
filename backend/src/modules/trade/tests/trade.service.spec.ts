...
describe("TradeService", () => {
  ...
  it("denies trade detail for non-participants", async () => {
    tradeRepository.findByTradeId.mockResolvedValue(mockTrade as never);
  ...
  it("returns trade detail for participants", async () => {
    ...
    const result = await service.getTradeById(
      "TRD-TEST-001",
      "seller",
      UserRole.TRADER,
    );
  ...
  it("throws when stock symbol not found", async () => {
    ...
    await expect(service.listTradesBySymbol("UNKNOWN", {})).rejects.toThrow(
      NotFoundException,
    );
  });
});
